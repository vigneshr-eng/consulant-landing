...
import instant from '../assests/instant.png'
import custom from '../assests/custom.png'

const FEATURES = [
  {
    title: 'Trusted Consulting Network',
    description: 'We carefully verify every consultant to help businesses connect with trusted and qualified experts.',
  },
  {
    title: 'Instant Expert Booking',
    description: 'Check consultant availability in real time and book appointments easily, without the hassle of endless emails or scheduling conflicts.',
  },
  {
    title: 'Secure Networking Platform',
    description: 'Keep your information safe with secure transactions, verified consultant profiles, and trusted communication tools.',
  },
  {
    title: 'Reach Global Networks',
    description: 'Connect with trusted professionals across different industries, specialisations, and locations, all from one platform.',
  },
  {
    title: 'Custom Consultation Experience',
    description: 'From one-time consultations and project support to coaching, strategic advisory, and long-term partnerships.',
  },
  {
    title: 'Created for Success',
    description: 'From finding the right expertise to building long-term partnerships, Bizpole Consult helps consultants and businesses grow together.',
  },
];

const OVERLAY_SRCS = [
  trust,    // Card 01 — Trusted Consulting Network
  instant,  // Card 02 — Instant Expert Booking
  secure,   // Card 03 — Secure Networking Platform
  reach,    // Card 04 — Reach Global Networks
  custom,   // Card 05 — Custom Consultation Experience
  create,   // Card 06 — Created for Success
];
...
export default function WhyChooseSection({ id }: Props) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (!topBelowNavbar && prevTopBelowNavbar) { setActiveIdx(0); activeIdxRef.current = 0; lockBody(window.scrollY); }
      if (!pastSection   && prevPastSection)    { setActiveIdx(FEATURES.length - 1); activeIdxRef.current = FEATURES.length - 1; lockBody(window.scrollY); }
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      if (e.deltaY > 0) {
        if (activeIdxRef.current < FEATURES.length - 1) {
          ...
          setActiveIdx((n) => { const v = n + 1; activeIdxRef.current = v; return v; });
  ...
  return (
    <section id={id} ref={sectionRef} className="w-full" style={{ background: '#284AA3' }}>
      <div
        className="mx-auto flex flex-col lg:flex-row gap-8 lg:gap-10 items-start lg:items-center"
        style={{ maxWidth: 1200, padding: 'clamp(28px, 5vw, 80px) clamp(16px, 4vw, 50px)' }}
      >
        {/* ── Left panel ── */}
        ...
        {isDesktop ? (
          ...
          <div className="flex-1 relative overflow-hidden" style={{ height: CONTAINER_H }}>
            ...
            <div style={{ transform: `translateY(${centerOffset(activeIdx)}px)`, transition: 'transform 0.52s cubic-bezier(0.33,1,0.68,1)' }}>
              {FEATURES.map((feat, i) => {
                ...
                return (
                  ...
                  >
                    {/* Number — top left */}
                    <span
                      className="absolute top-4 left-5 font-black text-black/10"
                    ...
                    {/* Text content */}
                    <div className="z-10" style={{ maxWidth: '50%' }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
            {FEATURES.map((feat, i) => (
              <div
                ...
                style={{ background: '#F3C625', border: '0.84px solid #525252', borderRadius: 17, padding: 18, minHeight: 220 }}
              >
                {/* Number — top left */}
                <span
                  className="absolute top-3 left-4 font-black text-black/10"
